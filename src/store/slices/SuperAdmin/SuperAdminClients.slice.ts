...
export const getSuperAdminClientsList = createAsyncThunk(
	'superAdminClients/getSuperAdminClientsList',
	async (
		{
			page,
			limit,
		...
		try {
			const response = await axiosInstance.get(
				`/client/list?page=${page}&limit=${limit}&search=${search}${searchBy && `&searchBy=${searchBy}`}`,
			);
...
export const getClientDetails = createAsyncThunk(
	'superAdminClients/getClientDetails',
...
export const getAgencyClientsWithJobs = createAsyncThunk(
	'superAdminClients/getAgencyClientsWithJobs',
...
export const inviteClient = createAsyncThunk(
	'superAdminClients/inviteClient',
...
export const assignJobToClient = createAsyncThunk(
	'superAdminClients/assignJobToClient',
...
export const unAssignJobToClient = createAsyncThunk(
	'superAdminClients/unAssignJobToClient',
...
export const getClientFeedback = createAsyncThunk(
	'superAdminClients/getClientFeedback',
	async (
		{
			page = 1,
			limit = 10,
...
export const superAdminClientsSlice = createSlice({
	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(getSuperAdminClientsList.pending, (state) => {
				state.pageLoading = true;
				state.error = null;
			})
			.addCase(getSuperAdminClientsList.fulfilled, (state, action) => {
				state.pageLoading = false;
				state.paginatedClients = action.payload.rows;
				state.paginationCount = action.payload.count;
			})
			.addCase(getSuperAdminClientsList.rejected, (state, action: any) => {
				state.pageLoading = false;
				state.error = action.payload || {
					message: 'Unknown error occurred while inviting client',
				};
...
export const { assignClientWhileCreatingJob, searchStoredClients, setClientSearch } =
	superAdminClientsSlice.actions;
export default superAdminClientsSlice.reducer;
