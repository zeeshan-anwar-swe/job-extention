...
export const getTaskBoardBackLogJobs = createAsyncThunk(
  ...
  async (
    { limit, page, startDate, endDate }: ApiParamsType,
    { rejectWithValue },
  ) => {
    try {
...
export const getTeamTaskBoardBackLogJobs = createAsyncThunk(
  "taskBoard/getTeamTaskBoardBackLogJobs",
  async (
    { limit, page, startDate, endDate }: ApiParamsType,
    { rejectWithValue },
  ) => {
    try {
      const response = await axiosInstance.get(
        `/team/taskboard?page=${page}&limit=${limit}&status=${JobStatus.BACKLOG}${`&startDate=${startDate ?? ""}`}${`&endDate=${endDate ?? ""}`}`,
      );
      return response.data.data;
    } catch (error: any) {
      return await withAsyncThunkErrorHandler(error, rejectWithValue);
...
export const getTeamTaskBoardInProgressJobs = createAsyncThunk(
  "taskBoard/getTeamTaskBoardInProgressJobs",
  async (
    { limit, page, startDate, endDate }: ApiParamsType,
    { rejectWithValue },
  ) => {
    try {
      const response = await axiosInstance.get(
        `/team/taskboard?page=${page}&limit=${limit}&status=${JobStatus.IN_PROGRESS}${`&startDate=${startDate ?? ""}`}${`&endDate=${endDate ?? ""}`}`,
      );
      return response.data.data;
    } catch (error: any) {
      return await withAsyncThunkErrorHandler(error, rejectWithValue);
...
export const getTeamTaskBoardInReviewJobs = createAsyncThunk(
  "taskBoard/getTeamTaskBoardInReviewJobs",
  async (
    { limit, page, startDate, endDate }: ApiParamsType,
    { rejectWithValue },
  ) => {
    try {
      const response = await axiosInstance.get(
        `/team/taskboard?page=${page}&limit=${limit}&status=${JobStatus.IN_REVIEW}${`&startDate=${startDate ?? ""}`}${`&endDate=${endDate ?? ""}`}`,
      );
      return response.data.data;
    } catch (error: any) {
      return await withAsyncThunkErrorHandler(error, rejectWithValue);
...
export const getTeamTaskBoardCompletedJobs = createAsyncThunk(
  "taskBoard/getTeamTaskBoardCompletedJobs",
  async (
    { limit, page, startDate, endDate }: ApiParamsType,
    { rejectWithValue },
  ) => {
    try {
      const response = await axiosInstance.get(
        `/team/taskboard?page=${page}&limit=${limit}&status=${JobStatus.COMPLETED}${`&startDate=${startDate ?? ""}`}${`&endDate=${endDate ?? ""}`}`,
      );
      return response.data.data;
    } catch (error: any) {
      return await withAsyncThunkErrorHandler(error, rejectWithValue);
...
export const changeMultipleJobsStatus = createAsyncThunk(
  ...
  async (
    { jobIds, status }: { jobIds: string[]; status: JobStatus },
    { rejectWithValue },
  ) => {
    try {
...
export const taskBoardSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      .addCase(getTeamTaskBoardBackLogJobs.pending, (state) => {
        state.backlogJobs.loading = true;
        state.backlogJobs.error = null;
      })
      .addCase(getTeamTaskBoardBackLogJobs.fulfilled, (state, action) => {
        state.backlogJobs.rows = action.payload.rows;
        state.backlogJobs.count = action.payload.count;
        state.backlogJobs.loading = false;
      })
      .addCase(getTeamTaskBoardBackLogJobs.rejected, (state, action: any) => {
        state.backlogJobs.error = action.payload || {
          message: "Unknown error occurred ",
        };
        toast.error(
    ...
    builder
      .addCase(getTeamTaskBoardInProgressJobs.pending, (state) => {
        state.inProgressJobs.loading = true;
        state.inProgressJobs.error = null;
      })
      .addCase(getTeamTaskBoardInProgressJobs.fulfilled, (state, action) => {
        state.inProgressJobs.rows = action.payload.rows;
        state.inProgressJobs.count = action.payload.count;
        state.inProgressJobs.loading = false;
      })
      .addCase(
        getTeamTaskBoardInProgressJobs.rejected,
        (state, action: any) => {
          state.inProgressJobs.error = action.payload || {
            message: "Unknown error occurred ",
          };
    ...
    builder
      .addCase(getTeamTaskBoardInReviewJobs.pending, (state) => {
        state.inReviewJobs.loading = true;
        state.backlogJobs.error = null;
      })
      .addCase(getTeamTaskBoardInReviewJobs.fulfilled, (state, action) => {
        state.inReviewJobs.rows = action.payload.rows;
        state.inReviewJobs.count = action.payload.count;
        state.inReviewJobs.loading = false;
      })
      .addCase(getTeamTaskBoardInReviewJobs.rejected, (state, action: any) => {
        state.inReviewJobs.error = action.payload || {
          message: "Unknown error occurred ",
        };
        toast.error(
    ...
    builder
      .addCase(getTeamTaskBoardCompletedJobs.pending, (state) => {
        state.completedJobs.loading = true;
        state.completedJobs.error = null;
      })
      .addCase(getTeamTaskBoardCompletedJobs.fulfilled, (state, action) => {
        state.completedJobs.rows = action.payload.rows;
        state.completedJobs.count = action.payload.count;
        state.completedJobs.loading = false;
      })
      .addCase(getTeamTaskBoardCompletedJobs.rejected, (state, action: any) => {
        state.completedJobs.error = action.payload || {
          message: "Unknown error occurred ",
        };
        toast.error(
