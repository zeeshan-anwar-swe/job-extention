...
import ModalInterfaceMD from './md/ModalInterface.md';
import ModalExample1MD from './md/ModalExample1.md';
...
const ModalPage = () => {
	...
	const [fullScreen, setFullScreen] = useState<IModalProps['fullScreen']>(
		Modal.defaultProps?.fullScreen,
	);
	const [isAnimation, setIsAnimation] = useState<IModalProps['isAnimation']>(
		Modal.defaultProps?.isAnimation,
	);
	const [isCentered, setIsCentered] = useState<IModalProps['isCentered']>(
		Modal.defaultProps?.isCentered,
	);
	const [isScrollable, setIsScrollable] = useState<IModalProps['isScrollable']>(
		Modal.defaultProps?.isScrollable,
	);
	const [isStaticBackdrop, setIsStaticBackdrop] = useState<IModalProps['isStaticBackdrop']>(
		Modal.defaultProps?.isStaticBackdrop,
	);
	const [rounded, setRounded] = useState<IModalProps['rounded']>(Modal.defaultProps?.rounded);
	const [size, setSize] = useState<IModalProps['size']>(Modal.defaultProps?.size);
	...
	const setModalPropsDefault = () => {
		...
		setFullScreen(Modal.defaultProps?.fullScreen);
		setIsAnimation(Modal.defaultProps?.isAnimation);
		setIsCentered(Modal.defaultProps?.isCentered);
		setIsScrollable(Modal.defaultProps?.isScrollable);
		setIsStaticBackdrop(Modal.defaultProps?.isStaticBackdrop);
		setRounded(Modal.defaultProps?.rounded);
		setSize(Modal.defaultProps?.size);
	};

	return (
		<PageWrapper name='Modal'>
			<Subheader>
				<SubheaderLeft>
				...
				<Doc>
					<DocContent>
						...
						<div className='col-span-12 md:col-span-4'>
							<ExampleView title='Example 1' mdFile={ModalExample1MD as RequestInfo}>
								<Card>
									<CardBody>
						...
						<div className='col-span-12 md:col-span-4'>
							<ExampleView title='Example 2' mdFile={ModalExample1MD as RequestInfo}>
								<Card>
									<CardBody>
						...
						<div className='col-span-12 md:col-span-4'>
							<ExampleView title='Example 3' mdFile={ModalExample1MD as RequestInfo}>
								<Card>
									<CardBody>
						...
						<div
							...
							<Card>
								...
								<CardBody>
									<MdViewer mdFile={ModalInterfaceMD as RequestInfo} />
								</CardBody>
							</Card>
						</div>
						<div
