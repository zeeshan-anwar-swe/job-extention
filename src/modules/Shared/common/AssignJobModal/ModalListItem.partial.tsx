...
const AssignJobModalListItemPartial = ({
  ...
  const { currentListPage } = useSelector(
  ...
  const isAssigned =
    assignToModule === "candidate"
      ? (job?.appliedCandidates
          ? job?.appliedCandidates
          : job?.candidateJobProfiles
        )?.some((candidate: any) => candidate.candidateId === assignTo) ||
        selfAssign
    ...
    try {
      ...
      if (isAssigned) {
        if (assignToModule === "client") {
          ...
          unAssignAction &&
            (await dispatch(
              unAssignAction({ jobId: job.id, unAssignTo: assignTo }),
            ));
        } else {
      ...
      if (userStorage.role === Roles.TEAM) {
        await dispatch(getTeamJobsList({ limit: 10, page: currentListPage }));
      } else {
  ...
  return (
    <div className="flex items-center justify-between gap-4 rounded-xl border pr-2">
      ...
      {assignToModule === "client" && !isJobSelfCreated && isAssigned ? (
        <Tooltip
          placement="top"
          text="You cannot Un Assign this job as it is created by client"
        >
          <Button
