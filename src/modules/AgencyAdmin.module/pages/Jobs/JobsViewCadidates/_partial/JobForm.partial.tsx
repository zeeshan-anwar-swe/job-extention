...
import { ExperienceRangeSelector } from "../../JobsCreateNewJob/_partial/ExperienceRangeSelector";
...
const JobFormPartial = ({ jobDetails }: any) => {
  ...
  const { componentLoading, error } = useSelector(
  ...
  useEffect(() => {
    if (jobDetails) {
      ...
      setFormData({
        ...
        spokenLanguages: jobDetails.spokenLanguages,
        experienceMin: jobDetails.experienceMin,
        experienceMax: jobDetails.experienceMax,
  ...
  const { loading, imageUrl } = useImageValidation(
    jobDetails?.client?.clientUser?.image,
  );

  const updateJobHandler = async () => {
    const description = await JSON.stringify(createDescendants(formData.description));
    await dispatch(
      updateJob({
        jobId: jobDetails.id,
  ...
  return (
    <Card
      className={cn(
        "col-span-5 flex !h-fit flex-col gap-2  p-4 max-lg:col-span-12",
      )}
    >
      <CardHeader>
      ...
      <CardHeader className=" !justify-start">
        {jobDetails?.client && (
          <Button
            ...
            className="!justify-start gap-2 !pl-2 !pr-2 "
          >
            ...
            <span>
              {textValidationCheck(jobDetails.client?.clientUser?.firstName)}
            </span>
            <span>
              {textValidationCheck(jobDetails.client?.clientUser?.lastName)}
            </span>
          </Button>
        )}
      ...
      <CardBody
        className={cn("!h-fit gap-4", !isEditPage && "pointer-events-none	")}
      >
