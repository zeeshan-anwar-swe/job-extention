      ...
      <PageWrapper name='Jobs'>
        ...
        <Subheader>
          <SubheaderLeft className='!block'>
            ...
            <CardSubTitle>Create, Delete, and assign candidates to jobs effectively.</CardSubTitle>
          </SubheaderLeft>
          <SubheaderRight>
            <div>
              <Checkbox
        ...
        <PageLoader messageForEmptyData={isDraft? "No draft jobs found":"No jobs found"} loading={pageLoading} error={error} data={paginatedList}>
          <Container className='grid grid-cols-12 gap-4'>
            {paginatedList.map((item: any) => (
              <JobsPageCardPartial isDraft={isDraft} item={item} key={item.id} />
            ))}
          </Container>
        </PageLoader>
        <Pagination
          searchBy={searchBy}
          search={search}
          getListAction={isDraft ? getDraftJobsList : getJobsList}
