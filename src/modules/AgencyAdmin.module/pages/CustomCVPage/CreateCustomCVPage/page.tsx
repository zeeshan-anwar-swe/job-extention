...
type TCVFormValues = {
  ...
  workExperience: {
    ...
    description: string;
    skills: string[]; // Added skills array to work experience
  }[];
};

const CreateCustomCVPage = () => {
  const { loading, data } = useSelector(
    (state: RootState) => state.customCV.cvDetails,
  );
  ...
  const formik = useFormik<TCVFormValues>({
    ...
    validate: (values) => {
      ...
      } else if (!isValidUrl(values.publicProfileUrl)) {
        errors.publicProfileUrl =
      ...
      // Validate work experience company URLs
      values.workExperience.forEach((exp, index) => {
        if (exp.companyUrl && !isValidUrl(exp.companyUrl)) {
          ...
          (errors.workExperience as any)[index] = {
            companyUrl: "Invalid URL it should start with http:// or https://",
          };
        }
      });
  ...
  // Helper to get work experience error
  const getWorkExperienceError = (index: number, field: string) => {
    if (
      formik.errors.workExperience &&
      Array.isArray(formik.errors.workExperience)
    ) {
      const expError = formik.errors.workExperience[index] as any;
      return expError?.[field];
    }
    return undefined;
  };
            ...
            <CardBody>
              ...
              >
                ...
                <div>
                  ...
                  >
                    <FieldWrap
                      firstSuffix={<Icon icon="HeroMapPin" className="mx-2" />}
                    >
                      <Input
                        dimension="lg"
                        id="location"
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            const [year, month] = e.target.value
                              .split("-")
                              .map(Number);
                            formik.setFieldValue(`education[${index}].start`, {
                              year,
                              month,
                            });
                          }}
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            const [year, month] = e.target.value
                              .split("-")
                              .map(Number);
                            formik.setFieldValue(`education[${index}].end`, {
                              year,
                              month,
                            });
                          }}
                ...
                {formik.values.workExperience.map((exp, index) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <Validation
                        isValid={!getWorkExperienceError(index, "companyUrl")}
                        isTouched={
                          formik.touched.workExperience?.[index]?.companyUrl
                        }
                        validFeedback="Good"
                        invalidFeedback={getWorkExperienceError(
                          index,
                          "companyUrl",
                        )}
                      >
                        <FieldWrap
                          firstSuffix={
                            <Icon icon="HeroLink" className="mx-2" />
                          }
                        >
                          <Input
                    ...
                    <div className="md:col-span-2">
                      <Label htmlFor={`workExperience[${index}].skills`}>
                        Skills Used
                      </Label>

                      <SelectReactCreateable
                        id={`workExperience[${index}].skills`}
                      ...
                      {/* <CreatableSelect
                        ...
                        onBlur={() => formik.setFieldTouched(`workExperience[${index}].skills`, true)}
