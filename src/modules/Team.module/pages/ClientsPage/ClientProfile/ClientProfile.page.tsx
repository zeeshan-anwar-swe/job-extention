...
const TeamClientProfilePage = () => {
  const { state } = useLocation();
  const { pageLoading, clientDetails, error } = useSelector(
          ...
          <Container className="!grid !grid-cols-12  !gap-4">
            ...
            <Card className="col-span-8 flex flex-col gap-2 max-lg:col-span-12">
              <CardHeader className="!block">
                ...
                <CardSubTitle>
                  Get an overview of the Client profile.
                </CardSubTitle>
              </CardHeader>
              <CardBody className="flex flex-col gap-4">
                ...
                <div className="flex items-center gap-4 max-md:flex-col ">
                  <LabelTitlepartial
                    label="Location"
                    detail={clientDetails?.clientUser.location}
                  />
                  <LabelTitlepartial
                    label="Company"
                    detail={clientDetails?.companyName}
                  />
                </div>
                <div className="flex items-center gap-4 max-md:flex-col ">
                  <LabelTitlepartial label="Designation" detail="" />
                  <LabelTitlepartial
              ...
              <CardFooter>
                {!clientDetails?.clientUser.linkedinUrl &&
                !clientDetails?.clientUser.githubUrl &&
                !clientDetails?.clientUser.twitterUrl &&
                !clientDetails?.clientUser.otherUrl ? null : (
                  <Label htmlFor="social-media" className="font-light">
                    Social Media
                  </Label>
                )}
                <CardFooterChild className="w-full">
                  ...
                  {clientDetails?.clientUser.otherUrl && (
                    <Button
                      className="gap-2"
                      icon="HeroGlobeAlt"
                      rightIcon="HeroArrowUpRight"
            ...
            <Card className="col-span-4 flex flex-col gap-2 max-lg:col-span-12">
              <CardHeader className="!flex-col !items-start gap-2">
                ...
                <p className="font-light">
                  View the jobs assigned to candidate.
                </p>
              </CardHeader>
...
export default TeamClientProfilePage;
